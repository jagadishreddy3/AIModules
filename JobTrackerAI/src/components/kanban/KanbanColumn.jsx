...
const KanbanColumn = ({ column, jobs, onEdit, onDelete }) => {
  ...
  return (
    <div className="flex flex-col w-72 shrink-0 h-full group/column">
      <div className="flex items-center justify-between mb-6 px-1">
        <div className="flex items-center gap-3">
          ...
          <span className="bg-jobflow-card border border-jobflow-border text-jobflow-text-dim text-xs font-black px-2 py-0.5 rounded-full min-w-[24px] text-center">
            {jobs.length}
          </span>
        </div>
        ...
        <SortableContext items={jobs.map((j) => j.id)} strategy={verticalListSortingStrategy}>
          {jobs.length > 0 ? (
            ...
            <div className="h-32 border-2 border-dashed border-jobflow-border rounded-[24px] flex flex-col items-center justify-center text-jobflow-text-dim/40 group transition-all hover:border-jobflow-accent/20">
              ...
              <span className="text-xs font-black uppercase tracking-widest">Drop here</span>
            </div>
          )}
        </SortableContext>
      </div>
