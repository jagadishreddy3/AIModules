...
import { ExternalLink, Edit2, Trash2, MapPin, Tag, Clock, Plus, Sparkles } from 'lucide-react';
    ...
    >
      <div className="flex justify-between items-start gap-4 mb-4 min-w-0">
        <div className="flex-1 min-w-0 overflow-hidden">
          <div className="flex items-center gap-2 mb-1 min-w-0">
            <span className="text-[10px] font-black text-jobflow-text-dim uppercase tracking-widest truncate shrink-0">
              {job.company}
            </span>
            <span className="text-jobflow-text-dim opacity-50 shrink-0">•</span>
            <span className="text-[10px] font-bold text-jobflow-text-dim flex items-center gap-1 truncate">
      ...
      <div className="flex flex-wrap gap-1.5 mb-5">
        {(job.tags || ['AI/ML', 'React']).slice(0, 3).map((tag, i) => (
          <span key={i} className="px-2 py-0.5 bg-jobflow-bg border border-jobflow-border text-[9px] font-black text-jobflow-text-dim rounded-md uppercase tracking-wider">
            {tag}
          </span>
        ))}
      </div>
      ...
      <div className="flex items-center justify-between border-t border-jobflow-border/50 pt-4 mt-auto">
        <div className="flex items-center gap-3">
          ...
          <button 
            className="flex items-center gap-1.5 px-2 py-1 bg-jobflow-accent/10 border border-jobflow-accent/20 rounded-lg text-jobflow-accent text-[9px] font-black uppercase tracking-widest hover:bg-jobflow-accent hover:text-white transition-all shadow-sm"
            onClick={(e) => { e.stopPropagation(); alert('AI Tailoring initiated for ' + job.company); }}
          >
